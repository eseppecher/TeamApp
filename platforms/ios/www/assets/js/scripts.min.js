...
};var old=$.fn.tab;$.fn.tab=function(option){return this.each(function(){var $this=$(this),data=$this.data("bs.tab");data||$this.data("bs.tab",data=new Tab(this)),"string"==typeof option&&data[option]()})},$.fn.tab.Constructor=Tab,$.fn.tab.noConflict=function(){return $.fn.tab=old,this},$(document).on("click.bs.tab.data-api",'[data-toggle="tab"], [data-toggle="pill"]',function(e){e.preventDefault(),$(this).tab("show")})}(jQuery),+function($){"use strict";var Affix=function(element,options){this.options=$.extend({},Affix.DEFAULTS,options),this.$window=$(window).on("scroll.bs.affix.data-api",$.proxy(this.checkPosition,this)).on("click.bs.affix.data-api",$.proxy(this.checkPositionWithEventLoop,this)),this.$element=$(element),this.affixed=this.unpin=this.pinnedOffset=null,this.checkPosition()};Affix.RESET="affix affix-top affix-bottom",Affix.DEFAULTS={offset:0},Affix.prototype.getPinnedOffset=function(){if(this.pinnedOffset)return this.pinnedOffset;this.$element.removeClass(Affix.RESET).addClass("affix");var scrollTop=this.$window.scrollTop(),position=this.$element.offset();return this.pinnedOffset=position.top-scrollTop},Affix.prototype.checkPositionWithEventLoop=function(){setTimeout($.proxy(this.checkPosition,this),1)},Affix.prototype.checkPosition=function(){if(this.$element.is(":visible")){var scrollHeight=$(document).height(),scrollTop=this.$window.scrollTop(),position=this.$element.offset(),offset=this.options.offset,offsetTop=offset.top,offsetBottom=offset.bottom;"top"==this.affixed&&(position.top+=scrollTop),"object"!=typeof offset&&(offsetBottom=offsetTop=offset),"function"==typeof offsetTop&&(offsetTop=offset.top(this.$element)),"function"==typeof offsetBottom&&(offsetBottom=offset.bottom(this.$element));var affix=null!=this.unpin&&scrollTop+this.unpin<=position.top?!1:null!=offsetBottom&&position.top+this.$element.height()>=scrollHeight-offsetBottom?"bottom":null!=offsetTop&&offsetTop>=scrollTop?"top":!1;if(this.affixed!==affix){this.unpin&&this.$element.css("top","");var affixType="affix"+(affix?"-"+affix:""),e=$.Event(affixType+".bs.affix");this.$element.trigger(e),e.isDefaultPrevented()||(this.affixed=affix,this.unpin="bottom"==affix?this.getPinnedOffset():null,this.$element.removeClass(Affix.RESET).addClass(affixType).trigger($.Event(affixType.replace("affix","affixed"))),"bottom"==affix&&this.$element.offset({top:scrollHeight-offsetBottom-this.$element.height()}))}}};var old=$.fn.affix;$.fn.affix=function(option){return this.each(function(){var $this=$(this),data=$this.data("bs.affix"),options="object"==typeof option&&option;data||$this.data("bs.affix",data=new Affix(this,options)),"string"==typeof option&&data[option]()})},$.fn.affix.Constructor=Affix,$.fn.affix.noConflict=function(){return $.fn.affix=old,this},$(window).on("load",function(){$('[data-spy="affix"]').each(function(){var $spy=$(this),data=$spy.data();data.offset=data.offset||{},data.offsetBottom&&(data.offset.bottom=data.offsetBottom),data.offsetTop&&(data.offset.top=data.offsetTop),$spy.affix(data)})})}(jQuery);var myApp=angular.module("myApp",["ngRoute","ngResource","LocalStorageModule","ngSanitize","angular-websql"]);myApp.config(["localStorageServiceProvider",function(localStorageServiceProvider){localStorageServiceProvider.setPrefix("bonoway")}]);var db;myApp.controller("mainController",function($scope,localStorageService,$location,$webSql){$scope.title="Homepage",db=$webSql.openDatabase("crewdb","1.0","Test DB",2097152),$scope.importData=function(){var dataLines=[];$.ajax({url:"datas/html/input.html",type:"get",async:!1,success:function(html){dataLines=angular.fromJson(String(html))}}),db.dropTable("lines"),db.createTable("lines",{id:{type:"INTEGER"},topo:{type:"TEXT"},name:{type:"TEXT"},grade:{type:"TEXT"},latitude:{type:"TEXT"},longitude:{type:"TEXT"},accuracy:{type:"INTEGER"},sector:{type:"TEXT"}});for(var i=0;i<dataLines.length;i++)db.insert("lines",{id:dataLines[i].id,topo:dataLines[i].topo,name:dataLines[i].name,grade:dataLines[i].grade,latitude:dataLines[i].latitude,longitude:dataLines[i].longitude,accuracy:dataLines[i].accuracy,sector:dataLines[i].sector}).then(function(results){console.log(results.insertId)});alert('data imported from "input.html"')},$scope.exportData=function(){$scope.lines=[],db.selectAll("lines").then(function(results){for(i=0;i<results.rows.length;i++)document.write(results.rows.item(i).id+";"),document.write(results.rows.item(i).name+";"),document.write(results.rows.item(i).latitude+";"),document.write(results.rows.item(i).longitude+";"),document.write(results.rows.item(i).accuracy+"<br>")})},$scope.showList=function(){$location.path("/list")},$scope.add_route=function(){$location.path("/add")}}),myApp.controller("ListCtrl",function($scope,$location){$scope.lines=[],db.selectAll("lines").then(function(results){for(i=0;i<results.rows.length;i++)$scope.lines.push(results.rows.item(i))}),$scope.back=function(){$location.path("/home")},$scope.detail=function(lineId){$location.path("/detail/"+lineId)}}),myApp.controller("DetailCtrl",function($scope,$routeParams,$location){id=parseInt($routeParams.lineId),gpsTriger(),$scope.pingGps=function(){gpsTriger()},$scope.line={},db.select("lines",{id:{value:id}}).then(function(results){$scope.line=results.rows.item(0)}),$scope.updating=function(){db.update("lines",{latitude:CurrentLatitude},{id:id}),db.update("lines",{longitude:CurrentLongitude},{id:id}),db.update("lines",{accuracy:CurrentAccuracy},{id:id}),$location.path("/list")},$scope.discard=function(){$location.path("/list")}}),myApp.controller("AddCtrl",function($scope,$location){$scope.back_home=function(){$location.path("/home")},$scope.back_list=function(){$location.path("/list")},$scope.save=function(name,grade){db.insert("lines",{id:0,topo:"",name:name,grade:grade,latitude:"",longitude:"",accuracy:0,sector:""}),$location.path("/add_saved")}}),myApp.config(["$routeProvider","$locationProvider",function($routeProvider,$locationProvider){$locationProvider.html5Mode(!1),$routeProvider.when("/",{templateUrl:"partials/default.html",controller:"mainController"}).when("/home",{templateUrl:"partials/home.html"}).when("/list",{templateUrl:"partials/list.html",controller:"ListCtrl"}).when("/detail/:lineId",{templateUrl:"partials/detail.html",controller:"DetailCtrl"}).when("/search",{templateUrl:"partials/search.html"}).when("/add",{templateUrl:"partials/add.html",controller:"AddCtrl"}).when("/add_saved",{templateUrl:"partials/add_saved.html",controller:"AddCtrl"}).otherwise({redirectTo:"/",controller:"mainController"})}]);