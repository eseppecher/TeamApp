...
};var old=$.fn.tab;$.fn.tab=function(option){return this.each(function(){var $this=$(this),data=$this.data("bs.tab");data||$this.data("bs.tab",data=new Tab(this)),"string"==typeof option&&data[option]()})},$.fn.tab.Constructor=Tab,$.fn.tab.noConflict=function(){return $.fn.tab=old,this},$(document).on("click.bs.tab.data-api",'[data-toggle="tab"], [data-toggle="pill"]',function(e){e.preventDefault(),$(this).tab("show")})}(jQuery),+function($){"use strict";var Affix=function(element,options){this.options=$.extend({},Affix.DEFAULTS,options),this.$window=$(window).on("scroll.bs.affix.data-api",$.proxy(this.checkPosition,this)).on("click.bs.affix.data-api",$.proxy(this.checkPositionWithEventLoop,this)),this.$element=$(element),this.affixed=this.unpin=this.pinnedOffset=null,this.checkPosition()};Affix.RESET="affix affix-top affix-bottom",Affix.DEFAULTS={offset:0},Affix.prototype.getPinnedOffset=function(){if(this.pinnedOffset)return this.pinnedOffset;this.$element.removeClass(Affix.RESET).addClass("affix");var scrollTop=this.$window.scrollTop(),position=this.$element.offset();return this.pinnedOffset=position.top-scrollTop},Affix.prototype.checkPositionWithEventLoop=function(){setTimeout($.proxy(this.checkPosition,this),1)},Affix.prototype.checkPosition=function(){if(this.$element.is(":visible")){var scrollHeight=$(document).height(),scrollTop=this.$window.scrollTop(),position=this.$element.offset(),offset=this.options.offset,offsetTop=offset.top,offsetBottom=offset.bottom;"top"==this.affixed&&(position.top+=scrollTop),"object"!=typeof offset&&(offsetBottom=offsetTop=offset),"function"==typeof offsetTop&&(offsetTop=offset.top(this.$element)),"function"==typeof offsetBottom&&(offsetBottom=offset.bottom(this.$element));var affix=null!=this.unpin&&scrollTop+this.unpin<=position.top?!1:null!=offsetBottom&&position.top+this.$element.height()>=scrollHeight-offsetBottom?"bottom":null!=offsetTop&&offsetTop>=scrollTop?"top":!1;if(this.affixed!==affix){this.unpin&&this.$element.css("top","");var affixType="affix"+(affix?"-"+affix:""),e=$.Event(affixType+".bs.affix");this.$element.trigger(e),e.isDefaultPrevented()||(this.affixed=affix,this.unpin="bottom"==affix?this.getPinnedOffset():null,this.$element.removeClass(Affix.RESET).addClass(affixType).trigger($.Event(affixType.replace("affix","affixed"))),"bottom"==affix&&this.$element.offset({top:scrollHeight-offsetBottom-this.$element.height()}))}}};var old=$.fn.affix;$.fn.affix=function(option){return this.each(function(){var $this=$(this),data=$this.data("bs.affix"),options="object"==typeof option&&option;data||$this.data("bs.affix",data=new Affix(this,options)),"string"==typeof option&&data[option]()})},$.fn.affix.Constructor=Affix,$.fn.affix.noConflict=function(){return $.fn.affix=old,this},$(window).on("load",function(){$('[data-spy="affix"]').each(function(){var $spy=$(this),data=$spy.data();data.offset=data.offset||{},data.offsetBottom&&(data.offset.bottom=data.offsetBottom),data.offsetTop&&(data.offset.top=data.offsetTop),$spy.affix(data)})})}(jQuery);var myAppModule=angular.module("myCamera",[]);myAppModule.factory("Camera",["$q",function($q){return{getPicture:function(options){var q=$q.defer();return navigator.camera.getPicture(function(result){q.resolve(result)},function(err){q.reject(err)},options),q.promise}}}]);var myApp=angular.module("myApp",["ngRoute","myCamera","ngResource","LocalStorageModule","ngSanitize","angular-websql","ui.sortable","parse-angular"]);myApp.config(["localStorageServiceProvider",function(localStorageServiceProvider){localStorageServiceProvider.setPrefix("bonoway")}]),Parse.initialize("N5nLFVFb7JVzDuVEpg93oKQ9JPNDFM8y4cz7Tm9F","5E8l7ETHeD2FaBj3IqpPUfGiV8ujDrUNhrGchA6j");var db,gpsoff=!0;myApp.controller("mainController",function($scope,localStorageService,$location,$webSql,Camera){var currentUser=Parse.User.current();currentUser||$location.path("/login"),$scope.title="Homepage",$scope.isActive=function(viewLocation){return viewLocation===$location.path()},db=$webSql.openDatabase("crewdb","1.0","Test DB",2097152),$scope.importData=function(){var dataSites,dataSectors,dataLines;$.ajax({url:"datas/html/input-sites.html",type:"get",async:!1,success:function(html){dataSites=angular.fromJson(String(html))}}),$.ajax({url:"datas/html/input-sectors.html",type:"get",async:!1,success:function(html){dataSectors=angular.fromJson(String(html))}}),$.ajax({url:"datas/html/input-lines.html",type:"get",async:!1,success:function(html){dataLines=angular.fromJson(String(html))}}),db.dropTable("sites"),db.dropTable("sectors"),db.dropTable("lines"),db.createTable("sites",{id:{type:"TEXT"},name:{type:"TEXT"},description:{type:"TEXT"},tags:{type:"TEXT"},couverture:{type:"TEXT"},latitude:{type:"TEXT"},longitude:{type:"TEXT"},volume:{type:"INTEGER"}}),db.createTable("sectors",{id:{type:"TEXT"},name:{type:"TEXT"},latitude:{type:"TEXT"},longitude:{type:"TEXT"},approach:{type:"TEXT"},volume:{type:"INTEGER"},site:{type:"INTEGER"}}),db.createTable("lines",{id:{type:"TEXT"},topo:{type:"TEXT"},name:{type:"TEXT"},grade:{type:"TEXT"},rate:{type:"INTEGER"},description:{type:"TEXT"},latitude:{type:"TEXT"},longitude:{type:"TEXT"},accuracy:{type:"INTEGER"},image:{type:"TEXT"},site:{type:"INTEGER"},sector:{type:"INTEGER"},rank:{type:"INTEGER"},date:{type:"INTEGER"}})},$scope.exportData=function(){$scope.lines=[],db.selectAll("lines").then(function(results){for(i=0;i<results.rows.length;i++)document.write(results.rows.item(i).id+";"),document.write(results.rows.item(i).name+";"),document.write(results.rows.item(i).grade+";"),document.write(results.rows.item(i).sector+";"),document.write(results.rows.item(i).rate+";"),document.write(results.rows.item(i).description+";"),document.write(results.rows.item(i).image+";"),document.write(results.rows.item(i).latitude+";"),document.write(results.rows.item(i).longitude+";"),document.write(results.rows.item(i).accuracy+"<br>")})},$scope.home=function(){$location.path("/home")},$scope.list=function(){$location.path("/list")},$scope.delete=function(lineId){$location.path("/delete"+lineId)},$scope.detail=function(lineId){$location.path("/detail"+lineId)},$scope.edit=function(lineId){$location.path("/edit"+lineId)},$scope.getPhoto=function(){Camera.getPicture().then(function(imageURI){var d=new Date,time=d.getTime();db.update("lines",{image:imageURI,date:time},{id:id}),$location.path("/line/"+id)},function(err){console.err(err)})}}),myApp.controller("HomeCtrl",function($scope,$location){db.selectAll("sites").then(function(results){for($scope.sites=[],i=0;i<results.rows.length;i++)$scope.sites.push(results.rows.item(i))}),$scope.detail=function(siteId){$location.path("/site/"+siteId)}}),myApp.controller("ServerCtrl",function($scope){var query=new Parse.Query("sites");query.find({success:function(results){$scope.result=results,$scope.loaded=!0},error:function(error){alert("Error: "+error.code+" "+error.message)}}),$scope.downloadSite=function(id){alert(id);var query=new Parse.Query("sites");query.equalTo("objectId",id),query.find({success:function(result){db.insert("sites",{id:result[0].id,name:result[0].get("name"),description:result[0].get("description"),tags:result[0].get("tags"),couverture:result[0].get("couverture"),latitude:result[0].get("latitude"),longitude:result[0].get("longitude"),volume:result[0].get("volume")}).then(function(results){alert("site rentré"),console.log(results.insertId)})}});var Sector=Parse.Object.extend("sectors"),Site=Parse.Object.extend("sites"),innerQuery=new Parse.Query(Site);innerQuery.equalTo("objectId",id);var query=new Parse.Query(Sector);query.matchesQuery("site",innerQuery),query.find({success:function(result){for(var i=0;i<result.length;i++)db.insert("sectors",{id:result[i].id,name:result[i].get("name"),latitude:result[i].get("latitude"),longitude:result[i].get("longitude"),approach:result[i].get("approach"),volume:result[i].get("volume"),site:result[i].get("site").id}).then(function(results){console.log(results.insertId)});alert("secteurs rentrés")}});var Line=Parse.Object.extend("lines"),Site=Parse.Object.extend("sites"),innerQuery=new Parse.Query(Site);innerQuery.equalTo("objectId",id);var query=new Parse.Query(Line);query.matchesQuery("site",innerQuery),query.find({success:function(result){$scope.resultat=result;for(var i=0;i<result.length;i++)db.insert("lines",{id:result[i].id,name:result[i].get("name"),grade:result[i].get("grade"),rate:result[i].get("rate"),description:result[i].get("description"),latitude:result[i].get("latitude"),longitude:result[i].get("longitude"),accuracy:result[i].get("accuracy"),site:result[i].get("site").id,sector:result[i].get("sector").id,image:result[i].get("image"),rank:result[i].get("rank"),date:result[i].createdAt}).then(function(results){console.log(results.insertId)});alert("voies rentrées")}})}}),myApp.controller("SendCtrl",function($scope){db.selectAll("sites").then(function(results){for($scope.sites=[],i=0;i<results.rows.length;i++)$scope.sites.push(results.rows.item(i))}),$scope.sendSite=function(id){$scope.sendData=[],$scope.sectors=[],$scope.lines=[],db.select("sites",{id:{value:id}}).then(function(results){$scope.sendData.push(results.rows.item(0)),db.select("sectors",{site:{value:id}}).then(function(results){for(var i=0;i<results.rows.length;i++)$scope.sectors.push(results.rows.item(i));$scope.sendData.push($scope.sectors),db.select("lines",{site:{value:id}}).then(function(results){for(var i=0;i<results.rows.length;i++)$scope.lines.push(results.rows.item(i));$scope.sendData.push($scope.lines)})})}),alert("send site under dev, id = "+id)}}),myApp.controller("SiteDetailCtrl",function($scope,$routeParams,$location){id=$routeParams.siteId;var sita;$scope.site={},db.select("sites",{id:{value:id}}).then(function(results){$scope.site=results.rows.item(0),sita=results.rows.item(0),$scope.sectors=[],db.select("sectors",{site:{value:id}}).then(function(results){for(var i=0;i<results.rows.length;i++)$scope.sectors.push(results.rows.item(i))})}),$scope.lineList=function(siteId,sectorId){$location.path("/site/"+siteId+"/sector/"+sectorId)}}),myApp.controller("SectorCtrl",function($scope,$routeParams,$location,$filter){id=$routeParams.siteId,idd=$routeParams.sectorId,$scope.name="",$scope.grade="6a",$scope.rate=-1,$scope.description="",$scope.sector=idd;var sita;$scope.currency=idd,$filtering=function(items,x){$result=[];for(var i=0;i<items.length;i++){var item=items[i];item.sector===x&&$result.push(item)}return $result},$scope.site={},$scope.list=[],$scope.lines=[],$scope.sectors=[],db.select("sites",{id:{value:id}}).then(function(results){$scope.site=results.rows.item(0),sita=results.rows.item(0),db.select("sectors",{site:{value:id}}).then(function(results){for(var i=0;i<results.rows.length;i++)$scope.sectors.push(results.rows.item(i));db.select("lines",{site:{value:id}}).then(function(results){for(var i=0;i<results.rows.length;i++)$scope.list.push(results.rows.item(i)),results.rows.item(i).sector==idd&&$scope.lines.push(results.rows.item(i))})})}),$scope.select=function(xid){0===xid?($scope.current={id:0,name:"Toutes les voies"},$scope.lines=$scope.list):($scope.current={},db.select("sectors",{id:{value:xid}}).then(function(results){$scope.current=results.rows.item(0)}),$scope.lines=$filtering($scope.list,xid))},$scope.listing=function(siteId,sectorId){$location.path("/site/"+siteId+"/sector/"+sectorId)},$scope.detail=function(lineId){$location.path("/line/"+lineId)},$scope.back=function(siteId){$location.path("/site/"+siteId)},$scope.addLine=function(){$location.path("/add/"+id+"/sector/"+idd)},$scope.cancelAdd=function(){$scope.name="",$scope.grade="6a",$scope.rate=-1,$scope.description="",$scope.sector=idd,$("#addModal").modal("hide")},$scope.saveAdd=function(){{var d=new Date,time=d.getTime();$scope.description}alert($scope.description),db.insert("lines",{id:time,name:$scope.name,grade:$scope.grade,rate:$scope.rate,description:$scope.description,latitude:"",longitude:"",accuracy:1e3,sector:$scope.sector,site:id,image:"",date:time}),$("#addModal").modal("hide"),$("body").removeClass("modal-open"),$(".modal-backdrop").remove(),$location.path("/line/"+time)},$scope.startOrder=function(){$("#orderModal").modal("show"),$scope.listTemp=[],$scope.listTemp=$filter("orderBy")($scope.lines,"rank")},$scope.saveOrder=function(){$("#orderModal").modal("hide");for(var i=0;i<$scope.listTemp.length;i++)db.update("lines",{rank:i+1},{id:$scope.listTemp[i].id});$scope.lines=[],db.select("lines",{site:{value:id}}).then(function(results){for(var i=0;i<results.rows.length;i++)results.rows.item(i).sector==$scope.current.id&&$scope.lines.push(results.rows.item(i))})},$scope.cancelOrder=function(){$scope.lines=[],db.select("lines",{site:{value:id}}).then(function(results){for(var i=0;i<results.rows.length;i++)results.rows.item(i).sector==$scope.current.id&&$scope.lines.push(results.rows.item(i));$("#orderModal").modal("hide")})}}),myApp.controller("DataCtrl",function($scope,$routeParams,$location){$scope.localizing=!1,id=$routeParams.lineId,$scope.line={},$scope.sectors=[],$scope.edit={},db.select("lines",{id:{value:id}}).then(function(results){$scope.line=results.rows.item(0),$scope.name=results.rows.item(0).name,$scope.grade=results.rows.item(0).grade,$scope.rate=results.rows.item(0).rate,$scope.description=results.rows.item(0).drescription,$scope.sector=results.rows.item(0).sector,$scope.site=results.rows.item(0).site,$scope.accuracy=results.rows.item(0).accuracy;var siteId=results.rows.item(0).site;db.select("sectors",{site:{value:siteId}}).then(function(results){for(var i=0;i<results.rows.length;i++)$scope.sectors.push(results.rows.item(i))})}),$scope.updateEdit=function(name,sector,grade,rate,description){var d=new Date,time=d.getTime();db.update("lines",{name:name,grade:grade,rate:rate,description:description,sector:sector,date:time},{id:id}),$("#editModal").modal("hide")},$scope.cancelEdit=function(){$scope.name=$scope.line.name,$scope.grade=$scope.line.grade,$scope.rate=$scope.line.rate,$scope.description=$scope.line.drescription,$scope.sector=$scope.line.sector,$("#editModal").modal("hide")},$scope.delete_route=function(){db.del("lines",{id:id}),$("#deleteModal").modal("hide"),$("body").removeClass("modal-open"),$(".modal-backdrop").remove(),$location.path("/site/"+$scope.line.site+"/sector/"+$scope.line.sector)},$scope.backSector=function(){0==gpsoff?$("#backModal").modal("show"):$location.path("/site/"+$scope.site+"/sector/"+$scope.sector)},$scope.savePosition=function(redirection){gpsoff=!0,db.update("lines",{latitude:CurrentLatitude},{id:id}),db.update("lines",{longitude:CurrentLongitude},{id:id}),db.update("lines",{accuracy:CurrentAccuracy},{id:id}),$("#backModal").modal("hide"),$("body").removeClass("modal-open"),$(".modal-backdrop").remove(),1==redirection?$scope.backSector():($scope.accuracy=CurrentAccuracy,$scope.localizing=!1)},$scope.forgetPosition=function(){gpsoff=!0,$("#backModal").modal("hide"),$("body").removeClass("modal-open"),$(".modal-backdrop").remove(),$scope.backSector()},$scope.pingGps=function(){$scope.localizing=!0,gpsTriger()}}),myApp.controller("loginCtrl",function($scope,$http,$location){$scope.connect=function(){Parse.User.logIn($scope.username,$scope.password,{success:function(){$location.path("/home")},error:function(){alert("error")}})}}),myApp.controller("UserCtrl",function($scope,$http,$location){alert(angular.identity(Parse.User.current())),$scope.currentUser=angular.extend(Parse.User.current()),$scope.username=$scope.currentUser.get("username"),alert($scope.currentUser.username),angular.isObject(value),$scope.logOut=function(){Parse.User.logOut(),$location.path("/login")}}),myApp.config(["$routeProvider","$locationProvider",function($routeProvider,$locationProvider){$locationProvider.html5Mode(!1),$routeProvider.when("/",{templateUrl:"partials/default.html",controller:"mainController"}).when("/home",{templateUrl:"partials/home.html",controller:"HomeCtrl"}).when("/server",{templateUrl:"partials/server.html",controller:"ServerCtrl"}).when("/send",{templateUrl:"partials/send.html",controller:"SendCtrl"}).when("/site/:siteId",{templateUrl:"partials/site/detail.html",controller:"SiteDetailCtrl"}).when("/site/:siteId/sector/:sectorId",{templateUrl:"partials/site/list.html",controller:"SectorCtrl"}).when("/line/:lineId",{templateUrl:"partials/detail.html",controller:"DataCtrl"}).when("/user",{templateUrl:"partials/user.html",controller:"UserCtrl"}).when("/login",{templateUrl:"partials/login.html",controller:"loginCtrl"}).otherwise({redirectTo:"/",controller:"mainController"})}]);